...
function setupBase(mymap) {
  ...
  // setting the tile source to an invalid data URL
  base._setTileSrc = function (tile, src) {
    tile.src = src.includes("***") ? "data:," : src;
  };
  ...
  base.createTile = function (coords, done) {
    ...
    if (replacements) {
      ...
      return tile;
      // TODO: is there away to avoid making requests when
      // we know the tile doesn't exist (i.e. the src link contains '***')
    } else {
