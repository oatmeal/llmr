  ...
  if (dataLayer.getLayers().length === 0) {
    if (data.markers) {
      ...
      for (const marker of data.markers) {
        ...
        // HACK: put gate markers on top
        if (url.endsWith("gate.json")) {
  ...
  mymap.sidebar.timelineControl.init = function (dates) {
    ...
    function updateLeftRight() {
      ...
      async function onclickHelper(nextIndex) {
        ...
        if (!monthDivs[mymap.dimData.timeline.date.slice(0, 6)].hasAttribute("open")) {
          monthDivs[mymap.dimData.timeline.date.slice(0, 6)].open = true;
        }
    ...
    for (const date of dates) {
      if (!monthDivs[date.slice(0, 6)]) {
        const monthDetails = timelineRadio.appendChild(
        ...
        summary.textContent = `${date.slice(0, 4)}年${date.slice(4, 6)}月`;
        monthDivs[date.slice(0, 6)] = monthDetails;
      }

      const dateEl = monthDivs[date.slice(0, 6)].appendChild(
        document.createElement("div")
      );
      ...
      inputEl.onchange = async function () {
        if (inputEl.checked === true) {
          if (!monthDivs[date.slice(0, 6)].hasAttribute("open")) {
            monthDivs[date.slice(0, 6)].open = true;
            inputEl.scrollIntoView({ block: "nearest" });
          }
        }
